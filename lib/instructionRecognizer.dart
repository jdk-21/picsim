  ...
  void recognize(String instruction) {
    ...
    if (instruction.startsWith("11111")) {
      ...
      int sum  = int.tryParse(
              ((int.parse(instruction.substring(6), radix: 2))
    ...
    // ANDLW
      ...
      int sum = int.parse(instruction.substring(6),radix: 2) & int.parse(wReg, radix: 2);
      String binSum = "00000000" + sum.toRadixString(2);
      wReg = binSum.substring(binSum.length-8);
    }
    // ADDWF
      ...
      int sum = int.parse(storage[address], radix: 16) + int.parse(wReg, radix: 2);
      if (instruction[6] == "0") {
        String binSum = "00000000" + sum.toRadixString(2);
        wReg = binSum.substring(binSum.length-8);
      } 
    ...
    // BCF
      ...
      int address = int.parse(instruction.substring(7), radix: 2);
      String d = "00000000" + int.parse(storage[address], radix: 16).toRadixString(2);
      String data = d.substring(d.length-8);
      String result = int.parse(data.substring(0, bit) + "0" + data.substring(bit + 1), radix: 2).toRadixString(16);
    ...
    // BSF
      ...
      String d = "00000000" + int.parse(storage[address], radix: 16).toRadixString(2);
      String data = d.substring(d.length-8);
      String result = "00" + int.parse(data.substring(0, bit) + "1" + data.substring(bit + 1), radix: 2).toRadixString(16);
      storage[address] = result.substring(result.length-2);
